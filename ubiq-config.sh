...
function advancedMenu() {
    ...
    case $ADVSEL in
        1)
            ...
            whiptail --title "Block Info" --msgbox "Please log in as 'pi' and run 'ubiq-config' again in order to perform this action." 8 47
        ...
        3)
            if [ -f "/home/pi/auto.sh" ]; then
                if [ "$(whoami)" != "pi" ] ; then
                whiptail --title "gubiq Update" --msgbox "Please log in as 'pi' and run 'ubiq-config' again in order to perform this action." 8 47
                ...
                if ( whiptail --title "gubiq Update" --yesno "Would you like to update gubiq now?" 8 40); then
                wget https://raw.githubusercontent.com/maaatttt/ubiq/master/gu.sh
                chmod +x gu.sh
                ./gu.sh
        ...
        5)
            if [ "$(whoami)" != "pi" ]; then
            whiptail --title "Shinobi Interface Installation" --msgbox "Please log in as 'pi' and run 'ubiq-config' again to perform this action." 8 47
            elif [ "$(whoami)" = "pi" ]; then
